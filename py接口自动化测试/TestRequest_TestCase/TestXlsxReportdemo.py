...
from TestAllRunner import thread
# 初始化测试数据
from initdata import init_data
# 把GetTestReport方法自己写出来
from testdata.getpath import GetTestReport, GetTestConfig
...
<table width='800' border='0' cellspacing='0' cellpadding='4'>
    <tr>
        <td bgcolor="#CECFAD" height='20' style='font-size:20px'>接口自动化测试报告 </td>
    </tr>
    <tr>
        <td bgcolor='#EFEBDB' height='100' style='font-ize:13px'>
        ...
        6)测试时间：<font color=red> %(timenow)s</font><br><br>
                详情内容请看附件：<font color=red> %(neirong)s</font>
        </td>
    </tr>
"""
...
try:
    ...
    mymail.connect()
    mymail.login()
    mail_content = msg % dict(yonglizongshu=str(len(TestReport)), tongguoyongli=str(hpassnum), shibaiyongli=str(len(
        TestReport)-hpassnum), tongguolv=str(round(hpassnum/len(TestReport), 2)*100), test_net=testUrl, timenow=timenow, neirong='['+(now+'report.xls')+']')
